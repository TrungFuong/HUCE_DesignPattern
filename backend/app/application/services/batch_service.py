...
class BatchService:
    ...
    # ──────────── Chemical management cho batch ────────────

    async def set_batch_chemicals(self, batch_id: str, items) -> list[BatchChemical]:
        """Replace toàn bộ danh sách hóa chất của lô."""
        await self.get_by_id(batch_id)
        if self.batch_chemical_repository is None:
            raise ValueError("BatchChemicalRepository is not configured")
        # Validate mỗi chemical tồn tại
        if self.chemical_repository:
            for item in items:
                chem = await self.chemical_repository.find_by_id(item.chemical_id)
                if chem is None:
                    raise ValueError(f"Hóa chất {item.chemical_id} không tồn tại")
        # Soft-delete cũ, rồi save mới
        ...
        return await self.batch_chemical_repository.save_all(entities)
